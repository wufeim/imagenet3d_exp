...
from imagenet3d.utils import bin_to_continuous, pose_error, eval_classification
...
def train_one_epoch(model, dataloader, optimizer, scheduler, epo, cfg):
    ...
    for idx, sample in enumerate(dataloader):
        ...
        pose_targets = torch.stack([azimuth_idx, elevation_idx, theta_idx], dim=1).view(-1)
        cls_targets = sample['cate'].cuda(non_blocking=True)

        cls_out, pose_out = model(img)
        pose_out = pose_out.reshape(-1, cfg.multi_bin.num_bins)

        pose_loss = construct_class_by_name(**cfg.training.loss)(
            pose_out.view(-1, cfg.multi_bin.num_bins), pose_targets)
        cls_loss = construct_class_by_name(**cfg.training.loss)(cls_out, cls_targets)
        loss = pose_loss * cfg.training.pose_loss_weight + cls_loss * cfg.training.cls_loss_weight
...
def evaluate(model, dataloader, cfg, epo):
    ...
    pose_errors, categories, sample_names, classifications = [], [], [], []
    for i, sample in enumerate(dataloader):
        ...
        azimuth_pred = bin_to_continuous(np.argmax(pose_out[:, 0:num_bins].detach().cpu().numpy(), axis=-1), **cfg.multi_bin)
        elevation_pred = bin_to_continuous(np.argmax(pose_out[:, num_bins:2*num_bins].detach().cpu().numpy(), axis=-1), **cfg.multi_bin)
        theta_pred = bin_to_continuous(np.argmax(pose_out[:, 2*num_bins:3*num_bins].detach().cpu().numpy(), axis=-1), **cfg.multi_bin)
        cls_pred = np.argmax(cls_out.detach().cpu().numpy(), axis=-1).astype(np.int32)
        ...
        classifications += eval_classification(sample['cate'].detach().cpu().numpy().astype(np.int32), cls_pred).tolist()
    ...
    cls_acc = np.mean(np.array(classifications))

    if is_main_process():
        log_str = f'[Evaluate at epoch {epo+1}] pi_6_acc = {pi_6_acc*100:.2f}%, pi_18_acc = {pi_18_acc*100:.2f}%, med_err = {med_err:.2f}, mean_err = {mean_err:.2f}, cls_acc = {cls_acc*100:.2f}%'
        logging.info(log_str)
